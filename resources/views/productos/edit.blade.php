    ...
    <div class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-2 text-sm text-gray-600">Modifica la información del producto</p>
            </div>
        ...
        <div class="bg-white shadow-sm rounded-lg p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $producto->nombre) }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-gray-900 focus:border-gray-900">
                    @error('nombre')
                ...
                <div>
                    ...
                    <select name="categoria" id="categoria" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-gray-900 focus:border-gray-900">
                        ...
                        <option value="impresion" {{ old('categoria', $producto->categoria) == 'impresion' ? 'selected' : '' }}>Impresión</option>
                        <option value="fotocopia" {{ old('categoria', $producto->categoria) == 'fotocopia' ? 'selected' : '' }}>Fotocopia</option>
                    </select>
                    @error('categoria')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="precio_venta" id="precio_venta" step="0.01" min="0" value="{{ old('precio_venta', $producto->precio_venta) }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-gray-900 focus:border-gray-900">
                    @error('precio_venta')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="flex items-center">
                        <input type="checkbox" name="activo" value="1" {{ old('activo', $producto->activo) ? 'checked' : '' }} class="rounded border-gray-300 text-gray-900 focus:ring-gray-900">
            ...
            <div class="mt-6">
                ...
                <textarea name="descripcion" id="descripcion" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-gray-900 focus:border-gray-900">{{ old('descripcion', $producto->descripcion) }}</textarea>
                @error('descripcion')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="maquina_id" id="maquina_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-gray-900 focus:border-gray-900">
                        ...
                        @foreach($maquinas as $maquina)
                            <option value="{{ $maquina->id }}" {{ old('maquina_id', $producto->maquina_id) == $maquina->id ? 'selected' : '' }}>
                                {{ $maquina->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="tipo_impresion" id="tipo_impresion" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-gray-900 focus:border-gray-900">
                        ...
                        <option value="b&n" {{ old('tipo_impresion', $producto->tipo_impresion) == 'b&n' ? 'selected' : '' }}>Blanco y Negro</option>
                        <option value="color" {{ old('tipo_impresion', $producto->tipo_impresion) == 'color' ? 'selected' : '' }}>Color</option>
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="insumo_id" id="insumo_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-gray-900 focus:border-gray-900">
                        ...
                        @foreach($insumos as $insumo)
                            <option value="{{ $insumo->id }}" {{ old('insumo_id', $producto->insumo_id) == $insumo->id ? 'selected' : '' }}>
